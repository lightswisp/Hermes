...
class WSClient
	...
	def initialize()
		...
		trap "SIGINT" do
			...
			@browser.quit rescue puts("Browser is already closed.")
			exit
		end
	...
	def get_data()
		return @driver.execute_script("return window.rx_queue.shift()")
	end

	def send_data(msg)
		return @driver.execute_script("return window.tx_queue.push('#{msg}')")
	end
end
